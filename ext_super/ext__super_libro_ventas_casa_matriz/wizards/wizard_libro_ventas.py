    ...
    def print_facturas(self):
        ...
        if cursor_compania:
            for comp in cursor_compania:
                self.get_invoice(action,comp.id)
        self.get_invoice(action,self.env.company.id)
        action="factura"
        cursor_compania = self.env['res.company'].search([('parent_id','=',self.env.company.id)])
        if cursor_compania:
            for com in cursor_compania:
                self.get_invoice(action,comp.id)#loca14
        self.get_invoice(action,self.env.company.id)#loca14
        return {'type': 'ir.actions.report','report_name': 'libro_ventas.reporte_factura_clientes','report_type':"qweb-pdf"}


    def get_invoice(self,accion,comp_id):#loca14
