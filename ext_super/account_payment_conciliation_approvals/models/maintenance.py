...
class MaintenanceRequest(models.Model):
    ...
    #         sale_order_ids = maintenance_line_ids_line_ids.mapped('maintenance_id')
        ...
        if 'product_qty' in values:
            ...
            self.filtered(
                lambda r: r.state == 'process' and float_compare(
                    r.product_qty, values['product_qty'], precision_digits=precision) != 0)._update_line_quantity(values)
        previous_product_uom_qty = {line.id: line.product_qty for line in self}
