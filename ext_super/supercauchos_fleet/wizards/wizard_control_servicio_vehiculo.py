...
class FlotaControlServicio(models.TransientModel):
    ...
    lines_ids = fields.Many2many('fleet.wizard.vehicles.lines',string='Lineas')

    def print_ordenes(self):
        self.env['fleet.wizard.vehicles.lines'].search([]).unlink()
        self._get_lineas()
        return {'type': 'ir.actions.report','report_name': 'supercauchos_fleet.flota_control_servicio_vehiculo','report_type':"qweb-pdf"}
    ...
    def vehiculos_p(self):
        values = []
        temp_id = 0
        busqueda = self.env['fleet.wizard.vehicles.lines'].search([
            ('type_vehicle', '=', 'propio')
        ])
        for item in busqueda.sorted(key=lambda vehicle: vehicle.vehicle_id.id):
            busqueda2 = self.env['fleet.wizard.vehicles.lines'].search([
                ('type_vehicle', '=', 'propio'),
            ...
            if temp_id != item.vehicle_id.id:
                temp_value = [vehicle, travels, fillers, traveled_km, days]
                values.append(temp_value)
                temp_id = item.vehicle_id.id
        return values
    ...
    def _get_lineas(self):
        t = self.env['fleet.wizard.vehicles.lines']
    ...
    def generate_xls_report(self):
        ...
        ws1 = wb1.add_sheet('Control de Servicios de Vehículos')
        fp = BytesIO()
        ...
        ws1.write_merge(row,row, 2, 3, "Control de Servicios de Vehículos", header_content_style)
        ...
        ws1.write(row,col+1,"Vehículo",sub_header_style_c)
        ws1.col(col+1).width = int((len('Vehículo')+25)*256)
        ws1.write(row,col+2,"Cant Viaje",sub_header_style_c)
        ws1.col(col+2).width = int((len('Cant Viaje')+10)*256)
        ws1.write(row,col+3,"Filler",sub_header_style_c)
        ws1.col(col+3).width = int((len('Filler')+14)*256)
        ...
        self.write({'state': 'get', 'report': out, 'name':'Control de Servicios de Vehículos '+ fecha+'.xls'})
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'fleet.wizard.vehicles',
            'view_mode': 'form',
            'view_type': 'form',
            'res_id': self.id,
            'views': [(False, 'form')],
