...
class HrRequisiciones(models.Model):
    ...
    fecha = fields.Date()
    departamento_solicitante = fields.Many2one('hr.department',track_visibility='onchange')
    jefe_compras_id = fields.Many2one('hr.employee',track_visibility='onchange')
    motivo = fields.Text()
    requisicion_line_ids = fields.One2many('hr.requisiciones.line', 'requisicion_id', string='Requisiciones',track_visibility='onchange')
    company_id = fields.Many2one('res.company','Company',default=lambda self: self.env.company.id)
    state=fields.Selection(selection=[('draft','En Borrador'),('posted','Confirmada')],default='draft',track_visibility='onchange')
