...
class XmlLines(models.Model):
    ...
    def generar_xml(self):
        ...
        rif= self.env.company.partner_id.doc_type+self.env.company.vat
        rif=rif.replace('-', '')
        ...
        elemento_1 = ET.Element('RelacionRetencionesISLR',RifAgente=rif.upper(),Periodo=periodo)
    ...
    def create_xml(self):
        ...
        for item in retencion:
            for line in item.lines_id:
                if item.invoice_id.marter_partner_id.id:
                    doc = str(item.invoice_id.marter_partner_id.doc_type)
                    if item.invoice_id.marter_partner_id.vat:
                        doc += '{0:^7}'.format(str(item.invoice_id.marter_partner_id.vat)) 
                    else :
                        raise ValidationError('el Proveedor Master : ' + item.invoice_id.marter_partner_id.name + ' no posee numero de documento' + " de la factura: " + item.invoice_id.invoice_number + " id :" + str(item.invoice_id.id)   )
                    # doc += '{0:^7}'.format(str(0))
                else :
                    doc = str(item.partner_id.doc_type)
                    if  item.partner_id.vat:
                        doc += '{0:^7}'.format(str(item.invoice_id.partner_id.vat))
                    else :
                        raise ValidationError('el Proveedor : ' + item.invoice_id.partner_id.name + ' no posee numero de documento' + " de la factura: " + item.invoice_id.invoice_number + " id :" + str(item.invoice_id.id))
                    # doc += '{0:^7}'.format(str(0))

                entero=int(line.base)
                decimal=round(line.base-entero,2)
                decimal=str(decimal)
                decimal=decimal.replace('.','')
                cuenta=decimal
                cuenta=len(decimal)
                if cuenta<=2:
                    valor=str(round(line.base,2))+"0"
                else:
                    valor=str(round(line.base,2))
                linea = self.env['account.xml.detalle.line'].create({
                'rif_retenido': str(doc).upper(),
                ...
                'monto_operacion' : valor,#str(round(line.base,2)),
