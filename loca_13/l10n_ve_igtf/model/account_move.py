...
class AccountMove(models.Model):
    ...
    def crea_asiento_anticipo_v2(self):
        cont=0
        monto_factura=round(abs(self.amount_residual_signed),4) # valores en bs
        for det in self.payment_ids:
            cont=cont+1
            nombre_anti=self.get_name_anticipo()
            monto_anticipo=round(det.saldo_disponible_signed,4) # valores en bs
            pasa='si'
            if monto_factura==0:
                ...
                if monto_factura>monto_anticipo:
                    ...
                    if self.currency_id.id!=self.env.company.currency_id.id:  #self.env.company.currency_id.id==det.currency_id.id and 
                        ...
                        if self.type=='out_invoice':
                            signo=-1
                        self.amount_residual=round(signo*(monto_factura/self.os_currency_rate),4)
                        self.amount_residual_signed=round(signo*(monto_factura),4)
                        if self.amount_residual==0.0: # AQUI VERIFICA SI NO QUEDA MILES DE DECIMALES Y PASARLO A ESTADO PAGADO
