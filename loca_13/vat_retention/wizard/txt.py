...
class BsoftContratoReport2(models.TransientModel):
    ...
    def action_generate_txt(self):
        ...
        rec_cursor = self.env['account.move'].search([('invoice_date','>=',self.date_from),('invoice_date','<=',self.date_to),('type','in',('in_invoice','in_refund','in_receipt')),('state','=','posted'),])
        ...
        #ruta="/home/odoo/src/user/LocalizacionV13/vat_retention/wizard/txt_generacion.txt"
                ...
                if rec.vat_ret_id:
                    if rec.vat_ret_id.state=="posted":
                        ...
                        if rec.type=="in_refund":
                            trans='03'
                        if rec.type=='in_receipt':
                            trans='02'
                        puntero_ret_line = self.env['vat.retention.invoice.line'].search([('retention_id','=',rec.vat_ret_id.id)])
                        for det_ret_line in puntero_ret_line:

                            if not rec.company_id.vat:
                                rif_compania="J-0000000000"
                            else:
                                rif_compania=rif_format(rec.company_id.vat,rec.company_id.partner_id.doc_type)
                            #rif_compania=rif_format(rec.company_id.vat)
                            #rif_compania="J-1234567890"
                            ...
                            rif_proveedor= rif_format(rec.partner_id.vat,rec.partner_id.doc_type)
                            file.write(rif_proveedor + "\t") #6

                            
                            invoicer_number=str(rec.invoice_number)
                            ...
                            total = str(det_ret_line.amount_untaxed+det_ret_line.amount_vat_ret)
                            total = completar_cero(total,12)
                            file.write(total + "\t") #9

                            importe_base = str(det_ret_line.amount_untaxed)
                            importe_base = completar_cero(importe_base,12)
                            file.write(importe_base + "\t") #10

                            monto_ret=str(det_ret_line.retention_amount)
                            monto_ret = completar_cero(monto_ret,12)
                            file.write(monto_ret + "\t") #11

                            #raise UserError(_('ref = %s')%rec.ref)
                            if rec.ref==False:
                                fact_afec='0'
                            else:
                                fact_afec = str(rec.ref)
                            fact_afec = completar_cero(fact_afec,5)
                            file.write(fact_afec + "\t") #12

                            nro_control = str(rec.vat_ret_id.name)
                            file.write(nro_control + "\t") #13
