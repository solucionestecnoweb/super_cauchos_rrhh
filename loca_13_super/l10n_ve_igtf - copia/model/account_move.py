...
class AccountMove(models.Model):
    ...
    def action_post(self):
        ...
        if self.usar_anticipo==True:
            ...
            for det in self.payment_ids:
                cont=cont+1
                acum=acum+det.saldo_disponible #self.cambio(det) 
                nombre_anti=self.get_name_anticipo()
                monto_factura=self.amount_residual_signed #self.amount_total
                #if cont==2:
                    #raise UserError(_('monto_factura = %s')%monto_factura)
                #monto_anticipo=self.payment_id.amount
                monto_anticipo=self.cambio(det) #det.saldo_disponible #self.cambio(det) 
    ...
    def registro_movimiento_linea_anticipo(self,id_movv,total_monto,anti_nombre,id_payment,acum_anti):
        ...
        anticipo_monto=self.cambio(id_payment) #id_payment.saldo_disponible #=self.cambio(id_payment)

        #residual=anticipo_monto  # funciona cuando el anticipor < factura

        if anticipo_monto<factura_monto:  #funciona cundo anticipo > factura
        ...
        valores=residual
        name = anti_nombre
        #raise UserError(_('valores 4 = %s')%valores)
...
#************ funcionpara que funcione en lanta *************
    def concilio_saldo_pendiente_anti(self,id_move_conci,id_payment,cont2,acum_anti):
        ...
        factura_monto=self.amount_total_signed #self.amount_total
        anticipo_monto=self.cambio(id_payment) #id_payment.saldo_disponible
        if anticipo_monto<factura_monto:
        ...
        if self.amount_residual_signed>0:
            ...
            value = {
                 ...
                 'amount':self.trampa(anticipo_monto),
                 'max_date':self.date,
                 #'currency_id':2,
            }
